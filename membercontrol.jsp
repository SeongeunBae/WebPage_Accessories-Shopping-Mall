...
<div id="wrap">
 ...
 <%
    ...
    try {
       ...
       memberList = dao.selectList(conn);
    }catch(SQLException e){ }
    finally {JdbcUtil.close(conn);}
    
%>
<p class="center"> 회원관리</p>
...
<tr>
 ...
 <td>${member.phoneNum1}-${member.phoneNum2}-${member.phoneNum3}</td>
 <td>${member.userId}</td>
 <td>${member.password}</td>
 <td>${member.userName}</td>
 <td>${member.email1}@${member.email2}</td>
 <td>${member.gender}</td>
 <td>${member.year}년${member.month}월${member.date}일</td>
 <td>${member.sample6_address}${member.daddress}</td>
 <td>${member.admitEmail}</td>
 <td>${member.registerTime}</td>
  <td><input type="button" onclick="location.href='memberUpdateForm.jsp?memberId=${member.userId}'" value="수정"/></td>
 <td><input type="button" onclick="location.href='memberDelete.jsp?memberId=${member.memberId}'" value="삭제"/></td>
 </tr>
</c:forEach>
</table>  
